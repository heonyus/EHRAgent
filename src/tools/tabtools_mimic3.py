...
DB_PATH = Path("data/ehrsql-ehragent/mimic_iii/mimic_iii.db")
###
# LoadDB는 원본처럼 SELECT * 로 전체 테이블을 DataFrame으로 만들기
###
...
def db_loader(target_ehr):
    if target_ehr not in LOADABLE:
        raise Exception(
            "LoadDB cannot load {}. Allowed tables: {}.".format(
                target_ehr,
                ", ".join(sorted(LOADABLE)),
            )
        )
    conn = sqlite3.connect(DB_PATH)
    data = pd.read_sql_query("SELECT * FROM {}".format(LOADABLE[target_ehr]), conn)
    conn.close()
    return data

# from github/EhrAgent/tools/tabtools.py
...
def date_calculator(argument):
    try:
        con = sqlite3.connect(DB_PATH)
        cur = con.cursor()
        command = 'select datetime((select "current_time" from "current_time"), "{}")'.format(argument)
        results = cur.execute(command).fetchall()[0][0]
    except Exception:
        raise Exception(
            "The date calculator {} is incorrect. Please check the syntax and make necessary changes. For the current date and time, please call Calendar('0 year').".format(argument)
        )
    return results